export default async function handler(req, res) {
  if (req.method === "POST") {
    // (your existing POST handler here)
    res.status(200).json({ success: true }); // stub for demo
    return;
  }
  ...
  if (req.method === "GET") {
    ...
    try {
      ...
      console.log("GHL Location API Response:", JSON.stringify(data, null, 2)); // debug print

      // Robust extraction: handle both array and object formats
      let boosterShotMsg = "";
